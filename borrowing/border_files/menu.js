...
function cm_makeStyle(){
	...
	if(this.usebar){
		...
		st='z-index:80; position:absolute; '
		if(!bw.ns6) str+='#div'+this.name+'Bar{'+st+'}\n'; else this.ns6styleb[this.ns6styleb.length]=st
		if(this.barinheritborder){str+='#div'+this.name+'Barb{z-index:85; position:absolute;}\n'}
	}
...
STM=new makeCoolMenu("STM") //Making the menu object. Argument: menuname
STM.useframes=0 //Do you want to use the menus as coolframemenu or not? (in frames or not) - Value: 0 || 1
STM.frame="frmMain" //The name of your main frame (where the menus should appear). Leave empty if you're not using frames - Value: "main_frame_name"

STM.useclick=0 //If you want the menu to be activated and deactivated onclick only set this to 1. - Value: 0 || 1
...
STM.offlineUrl="file:///C|/Inetpub/wwwroot/dhtmlcentral/" //Value: "path_to_menu_file_offline/"
...
STM.onlineUrl="http://www.dhtmlcentral.com/coolmenus/examples/withoutframes/" //Value: "path_to_menu_file_online/"


STM.pagecheck=1 //Do you want the menu to check whether any of the subitems are out of the bouderies of the page and move them in again (this is not perfect but it hould work) - Value: 0 || 1
STM.checkscroll=1 //Do you want the menu to check whether the page have scrolled or not? For frames you should always set this to 1. You can set this to 2 if you want this feature only on explorer since netscape doesn't support the window.onscroll this will make netscape slower (only if not using frames) - Value: 0 || 1 || 2
STM.resizecheck=1 //Do you want the page to reload if it's resized (This should be on or the menu will crash in Netscape4) - Value: 0 || 1
STM.wait=1000 //How long to wait before hiding the menu on mouseout. Netscape 6 is a lot slower then Explorer, so to be sure that it works good enough there you should not have this lower then 500 - Value: milliseconds

//Background bar properties
STM.usebar=1 //If you want to use a background-bar for the top items set this on - Value: 1 || 0
STM.barcolor="white" //The color of the background bar - Value: "color"
STM.barwidth="100%" //The width of the background bar. Set this to "menu" if you want it to be the same width as the menu. (this will change to match the border if you have one) - Value: px || "%" || "menu"
STM.barheight="menu" //The height of the background bar. Set this to "menu" if you want it to be the same height as the menu. (this will change to match the border if you have one) - Value: px || "%" || "menu"
STM.barx="menu" //The left position of the bar. Set this to "menu" if you want it be the same as the left position of the menu. (this will change to match the border if you have one)  - Value: px || "%" || "menu"
STM.bary="menu" //The top position of the bar Set this to "menu" if you want it be the same as the top position of the menu. (this will change to match the border if you have one)  - Value: px || "%" || "menu"
STM.barinheritborder=1 //Set this to 1 if you want the bar to have the same border as the top menus - Value: 0 || 1

//Placement properties
STM.rows=1 //This controls whether the top items is supposed to be laid out in rows or columns. Set to 0 for columns and 1 for row - Value 0 || 1
STM.fromleft=0 //This is the left position of the menu. (Only in use if menuplacement below is 0 or aligned) (will change to adapt any borders) - Value: px || "%"
STM.fromtop=18 //This is the left position of the menu. (Only in use if menuplacement below is 0 or aligned) (will change to adapt any borders) - Value: px || "%"
STM.pxbetween=0 //How much space you want between each of the top items. - Value: px || "%"
...
STM.level[0]=new Array() //Add this for each new level
STM.level[0].width=20 //The default width for each level[0] (top) items. You can override this on each item by spesifying the width when making the item. - Value: px || "%"
STM.level[0].height=20 //The default height for each level[0] (top) items. You can override this on each item by spesifying the height when making the item. - Value: px || "%"
STM.level[0].bgcoloroff="" //transparent The default background color for each level[0] (top) items. You can override this on each item by spesifying the backgroundcolor when making the item. - Value: "color"
STM.level[0].bgcoloron="" //The default "on" background color for each level[0] (top) items. You can override this on each item by spesifying the "on" background color when making the item. - Value: "color"
STM.level[0].textcolor="gray" //The default text color for each level[0] (top) items. You can override this on each item by spesifying the text color when making the item. - Value: "color"
STM.level[0].hovercolor="" //The default "on" text color for each level[0] (top) items. You can override this on each item by spesifying the "on" text color when making the item. - Value: "color"
STM.level[0].style="padding:1px; text-align:center; font-family:tahoma,arial,helvetica; font-size:12px; font-weight:bold" //The style for all level[0] (top) items. - Value: "style_settings"
STM.level[0].border=5 //The border size for all level[0] (top) items. - Value: px
STM.level[0].bordercolor="transparent" //The border color for all level[0] (top) items. - Value: "color"
STM.level[0].offsetX=-30 //The X offset of the submenus of this item. This does not affect the first submenus, but you need it here so it can be the default value for all levels. - Value: px
STM.level[0].offsetY=0 //The Y offset of the submenus of this item. This does not affect the first submenus, but you need it here so it can be the default value for all levels. - Value: px
STM.level[0].NS4font="tahoma,arial,helvetica"
STM.level[0].NS4fontSize="2"

/*New: Added animation features that can be controlled on each level.*/
STM.level[0].clip=0 //Set this to 1 if you want the submenus of this level to "slide" open in a animated clip effect. - Value: 0 || 1
STM.level[0].clippx=0 //If you have clip spesified you can set how many pixels it will clip each timer in here to control the speed of the animation. - Value: px 
STM.level[0].cliptim=0 //This is the speed of the timer for the clip effect. Play with this and the clippx to get the desired speed for the clip effect (be carefull though and try and keep this value as high or possible or you can get problems with NS4). - Value: milliseconds
...
STM.level[0].filter=0 //VALUE: 0 || "filter specs"
...
*/
STM.level[0].align="top" //Value: "top" || "bottom" || "left" || "right" 

//EXAMPLE SUB LEVEL[1] PROPERTIES - You have to spesify the properties you want different from LEVEL[0] - If you want all items to look the same just remove this
STM.level[1]=new Array() //Add this for each new level (adding one to the number)
STM.level[1].width=100
STM.level[1].height=22
STM.level[1].bgcoloroff="#F8F8F8"
STM.level[1].bgcoloron="#F1F1F1"
STM.level[1].style="padding:2px; font-family:tahoma, arial,helvetica; font-size:11px; font-weight:bold"
STM.level[1].align="right" 
STM.level[1].offsetX=0
STM.level[1].offsetY=15
STM.level[1].border=1 
STM.level[1].bordercolor="#006699"
STM.level[1].align="right" 

//EXAMPLE SUB LEVEL[2] PROPERTIES - You have to spesify the properties you want different from LEVEL[1] OR LEVEL[0] - If you want all items to look the same just remove this
STM.level[2]=new Array() //Add this for each new level (adding one to the number)
STM.level[2].width=STM.level[0].width
STM.level[2].height=20
STM.level[2].bgcoloroff="#F1F1F1"
STM.level[2].bgcoloron="#F8F8F8"
STM.level[1].textcolor=""
STM.level[2].style="padding:2px; font-family:tahoma,arial,helvetica; font-size:10px; font-weight:bold"
STM.level[2].align="right" 
STM.level[2].offsetX=0
STM.level[2].offsetY=23
STM.level[2].border=1 
STM.level[2].bordercolor="#006699"
STM.level[2].NS4font="tahoma,arial,helvetica"
STM.level[2].NS4fontSize="1"
STM.level[2].align="left" 
...
*/
STM.makeMenu('top0','','<img src=./images/STsm.gif  >','index.php?O=SSSC','parent')
	STM.makeMenu('sub00','top0','๏ อารัมภบท','index.php','parent')
	STM.makeMenu('sub01','top0','๏ อนุกรม','index.php?O=CISC','parent')
		STM.makeMenu('sub011','sub01','- สะเตมี : สารบัญ','index.php?O=CoSC','parent',160,0)
		STM.makeMenu('sub012','sub01','- สะเตมี : สารานุกรม','index.php?O=EncSC&seEnc=r','parent',160,0)
		STM.makeMenu('sub013','sub01','- สะเตมี : พจนานุกรม','index.php?O=DictISC','parent',160,0)
		STM.makeMenu('sub014','sub01','- สะเตมี : ปทานุกรม','index.php?O=GlosSC','parent',160,0)
		STM.makeMenu('sub015','sub01',"<a onclick=document.getElementById('DictShow').innerHTML=global.langs['SatemyMenuDictkw']; ><font color=gray>- สะเตมี : ดิคทุกที่</font></a>",'','parent',160,0)
	STM.makeMenu('sub02','top0','๏ ค้นพระไตรปิฎก','index.php?O=SSSC','parent')
		STM.makeMenu('sub021','sub02','- สะเตมี : ค้นพระไตรปิฎก','index.php?O=SSSC','parent',160,0)
		STM.makeMenu('sub022','sub02','- สะเตมี : บาลี - ไทย','index.php?O=PI&bdr=off','parent',160,0)
		STM.makeMenu('sub023','sub02','- สะเตมี : เรียนพระไตรปิฎก','index.php?O=TI&bdr=off','parent',160,0)
		STM.makeMenu('sub024','sub02','- สะเตมี : ทุกระบบ','index.php?O=AI&bdr=off','parent',160,0)
	STM.makeMenu('sub03','top0','๏ คู่มือ','index.php?O=GC','parent')
